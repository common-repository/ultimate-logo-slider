jQuery(function($) {
  ...
  $(document).on('click', '#ed-logo-carousel-metabox a.ed-logo-carousel-add', function(e) {
    ...
    file_frame.on('select', function() {
      ...
      selection.map(function(attachment, i) {
        ...
        //$('#ed-logo-carousel-metabox-list').append('<li><input type="hidden" name="ata_logo_id[' + index + ']" value="' + attachment.id + '"><img class="image-preview" src="' + attachment.sizes.thumbnail.url + '"><a class="change-image button button-small" href="#" data-uploader-title="Change image" data-uploader-button-text="Change image">Change image</a><br><small><a class="remove-image" href="#">Remove image</a></small></li>');
		$('#ed-logo-carousel-metabox-list').append('<li>'+
		'<input type="hidden" name="ata_logo_id[' + index + ']" value="' + attachment.id + '">'+
		'<img class="image-preview" src="' + attachment.sizes.thumbnail.url + '">'+
		'<div class="ed-pull-right">'+
		'<input type="text" name="ata_logo_link[' + index + ']" size="120" placeholder="Logo link" class="wdith-70" />'+
		'<select name="ata_logo_target[' + index + ']" class="wdith-30">'+
		'<option value="_blank">Blank</option>'+
		'<option value="_parent">Parent</option>'+
		'<option value="_self">Self</option>'+
		'<option value="_top">Top</option>'+
		'</select>'+
		'<input type="text" name="ata_logo_tooltip[' + index + ']"  size="120" placeholder="Tooltip Text (pro features)" readonly="readonly" />'+
		'</div>'+
  ...
  function resetIndex() {
    $('#ed-logo-carousel-metabox-list li').each(function(i) {
      $(this).find('input:hidden').attr('name', 'ata_logo_id[' + i + ']');
	  $(this).find('input:text').attr('name', 'ata_logo_link[' + i + ']');
	  $(this).find('select').attr('name', 'ata_logo_target[' + i + ']');
	  $(this).find('input:text').attr('name', 'ata_logo_tooltip[' + i + ']');
    });
  }
